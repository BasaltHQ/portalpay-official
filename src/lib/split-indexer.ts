...
export async function indexSplitTransactions(splitAddress: string, merchantWallet: string) {
  try {
    debug("SPLIT INDEXER", `Starting indexing for merchant ${merchantWallet.slice(0, 10)}...`);
    ...
    debug("SPLIT INDEXER", `Successfully indexed ${result.indexed} new transactions for ${merchantWallet.slice(0, 10)}...`);
...
export async function watchSplitContract(splitAddress: string, merchantWallet: string, onNewTransaction?: (tx: any) => void) {
  try {
    ...
    debug("SPLIT WATCHER", `Starting to watch split contract ${splitAddress.slice(0, 10)}...`);
    ...
    debug("SPLIT WATCHER", `Found ${events.length} historical PaymentReleased events`);
...
export async function indexAllMerchants() {
  try {
    debug("SPLIT INDEXER", `Starting batch indexing for all merchants...`);
