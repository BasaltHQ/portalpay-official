...
//   try {
//     if (isDemoMode()) {
//       debug('Toast Scheduler', 'Demo mode: skipping sync cycle');
//       return;
//     }
...
//       debug('Toast Scheduler', `Running employee sync for ${restaurantGuid}`);
//       const res = await syncEmployeesForRestaurant(restaurantGuid);
//       debug('Toast Scheduler', `Employee sync for ${restaurantGuid} -> processed=${res.processed} created=${res.created} updated=${res.updated} errors=${res.errors}`);
//     }
//   } catch (err) {
