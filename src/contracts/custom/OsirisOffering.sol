...
contract OsirisOffering {
    ...
    constructor(
        ...
        require(_payees_.length == _shares_.length, "OsirisOffering: length mismatch");
        require(_payees_.length > 0, "OsirisOffering: no payees");
        require(_platform != address(0), "OsirisOffering: platform is zero address");
    ...
    function releasable(address account) public view returns (uint256) {
        ...
        return _pendingPayment(account, totalReceived, _released[account], true, address(0));
    }
    ...
    function releasable(IERC20 token, address account) public view returns (uint256) {
        ...
        uint256 totalReceived = token.balanceOf(address(this)) + _totalReleasedERC20[address(token)];
        return _pendingPayment(account, totalReceived, _releasedERC20[address(token)][account], false, address(token));
    }

    // --- Release & Distribute ---

    function release(address payable account) public {
        require(_shares[account] > 0 || account == platform, "OsirisOffering: no shares");
    ...
    function _pendingPayment(address account, uint256 totalReceived, uint256 alreadyReleased, bool isETH, address token) private view returns (uint256) {
        ...
        if (isETH && ethTransactionCount > 0 && fixedFeeUSD > 0) {
             (int256 price, uint8 priceDecimals) = _getAssetPrice(true, address(0));
             if (price > 0) {
                 ...
                 if (priceDecimals != 8) {
                     if (priceDecimals < 8) denominator = denominator * (10**(8 - priceDecimals));
                     else denominator = denominator / (10**(priceDecimals - 8));
                 }
    ...
    function _getAssetPrice(bool isETH, address token) private view returns (int256, uint8) {
        AggregatorV3Interface feed = isETH ? ethUsdPriceFeed : tokenPriceFeeds[token];
    ...
    function getRoleMemberCount(bytes32 role) external view returns (uint256) { return 0; }
    function getRoleMember(bytes32 role, uint256 index) external view returns (address) { return address(0); }
    function getRoleAdmin(bytes32 role) external view returns (bytes32) { return DEFAULT_ADMIN_ROLE; }
