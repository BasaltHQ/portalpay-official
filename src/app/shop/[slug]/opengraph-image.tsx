 ...
 */
function generateStars(rating: number, x: number, y: number, size: number = 20, color: string = '#fbbf24'): string {
  ...
  return `
    <defs>
      <radialGradient id="meshGrad1" cx="0%" cy="0%" r="70%">
        <stop offset="0%" style="stop-color:${primary};stop-opacity:0.4" />
        <stop offset="100%" style="stop-color:${primary};stop-opacity:0" />
      </radialGradient>
      <radialGradient id="meshGrad2" cx="100%" cy="100%" r="70%">
        <stop offset="0%" style="stop-color:${secondary};stop-opacity:0.4" />
        <stop offset="100%" style="stop-color:${secondary};stop-opacity:0" />
      </radialGradient>
      <radialGradient id="meshGrad3" cx="50%" cy="30%" r="50%">
        <stop offset="0%" style="stop-color:${blendColor};stop-opacity:0.25" />
      ...
      <!-- Overlay gradient for cover photo -->
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  let coverPhotoMeta: { width: number; height: number } | null = null;
  let useFullWidthLayout = false; // false = sidepane, true = full-width top
  
  // Cover photo target dimensions
  const COVER_FULL_WIDTH = { width: CANVAS.width, height: 300 };
  const COVER_SIDEPANE = { width: 480, height: CANVAS.height };
  const COVER_CORNER_RADIUS = 16;
  const COVER_PADDING = 12; // Padding inside the matte area
  ...
  if (coverPhotoUrl) {
    try {
      ...
      console.log('[Shop OG Image] Loading cover photo from:', absoluteCoverUrl);
      ...
      if (rawCover) {
        ...
        const aspectRatio = coverPhotoMeta.width / coverPhotoMeta.height;
        useFullWidthLayout = aspectRatio >= 1.5; // Wide/landscape → full-width top
        
        console.log('[Shop OG Image] Cover photo aspect ratio:', aspectRatio.toFixed(2), 
        ...
        console.log('[Shop OG Image] Cover photo processed: target', targetDims.width, 'x', targetDims.height,
                    '| image', imgW, 'x', imgH, '| offset', offsetX, ',', offsetY);
      }
    } catch (err) {
      console.error('[Shop OG Image] Error loading cover photo:', err);
  ...
  const layout = useFullWidthLayout || !coverPhotoBuffer
    ? {
        ...
        shopLogo: { x: 60, y: coverPhotoBuffer ? 340 : 180, size: 140 },
        text: { 
          x: 220, 
          titleY: coverPhotoBuffer ? 380 : 220, 
          descStartY: coverPhotoBuffer ? 450 : 310, 
          lineHeight: 28,
          maxWidth: 900,
        },
        platformBadge: { symbolSize: 28 },
      }
    : {
        // Sidepane layout: content on left, cover on right
        shopLogo: { x: 50, y: 140, size: 120 },
        text: { 
          x: 190, 
          titleY: 170, 
          descStartY: 260, 
          lineHeight: 28,
          maxWidth: 450, // Narrower to leave room for cover
        },
        platformBadge: { symbolSize: 28 },
      };
  
  // Wrap description based on layout
  const descLines = wrapTextToLines(shopDescription, layout.text.maxWidth, 20, 3);
  ...
  if (shopLogoUrl) {
    try {
      ...
      console.error('[Shop OG Image] Error loading shop logo:', err);
      shopLogoBuffer = null;
    }
  }
  ...
  const svgContent = `
    <svg width="${CANVAS.width}" height="${CANVAS.height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- Accent bar at top -->
      <rect x="0" y="0" width="${CANVAS.width}" height="6" fill="url(#accentGrad)" />
      
      ${useFullWidthLayout && coverPhotoBuffer ? `
        <!-- Cover photo overlay gradient (for full-width layout) -->
        <rect x="0" y="0" width="${CANVAS.width}" height="300" fill="url(#coverOverlay)" />
      ` : ''}
      
      ${!useFullWidthLayout && coverPhotoBuffer ? `
        <!-- Sidepane overlay gradient (for sidepane layout) -->
        <rect x="${CANVAS.width - 480}" y="0" width="480" height="${CANVAS.height}" fill="url(#sidepaneOverlay)" />
      ` : ''}
      ...
      ${!shopLogoBuffer ? `
        ...
        <text x="${layout.shopLogo.x + layout.shopLogo.size / 2}" 
              y="${layout.shopLogo.y + layout.shopLogo.size / 2 + 32}" 
              font-family="system-ui, -apple-system, sans-serif" 
              font-size="64" 
              font-weight="700" 
              fill="${primaryColor}"
      ...
      <text x="${layout.text.x}" y="${layout.text.titleY}" 
            font-family="system-ui, -apple-system, sans-serif" 
            font-size="${useFullWidthLayout || !coverPhotoBuffer ? 48 : 40}" 
            font-weight="700" 
            fill="${textColor}"
            style="text-shadow: 0 2px 8px rgba(0,0,0,0.5);">
        ${escapeForSvg(shopName.length > 28 ? shopName.slice(0, 26) + '...' : shopName)}
      </text>
      
      <!-- Rating stars -->
      ${reviewCount > 0 ? `
        <g transform="translate(${layout.text.x}, ${layout.text.titleY + 25})">
          ${generateStars(avgRating, 0, 18, 16, '#fbbf24')}
          <text x="100" y="18" 
                font-family="system-ui, -apple-system, sans-serif" 
                font-size="14" 
                fill="${mutedColor}">
      ...
      <line x1="${layout.text.x}" y1="${layout.text.titleY + 60}" 
            x2="${layout.text.x + Math.min(layout.text.maxWidth, 600)}" y2="${layout.text.titleY + 60}" 
      ...
      <rect x="${layout.text.x}" y="${layout.text.descStartY + descLines.length * layout.text.lineHeight + 20}" 
            width="140" height="42" rx="6" 
            fill="${primaryColor}" />
      <text x="${layout.text.x + 70}" y="${layout.text.descStartY + descLines.length * layout.text.lineHeight + 20 + 28}" 
            font-family="system-ui, -apple-system, sans-serif" 
            font-size="16" 
            font-weight="600" 
      ...
      <g transform="translate(${!useFullWidthLayout && coverPhotoBuffer ? 50 : CANVAS.width - 220}, ${CANVAS.height - 45})">
        <text x="${layout.platformBadge.symbolSize + 6}" y="16" 
              font-family="system-ui, -apple-system, sans-serif" 
              font-size="13" 
  ...
  // Add cover photo (first so it's below other elements)
  if (coverPhotoBuffer) {
  ...
  if (platformSymbol) {
    const platformSymbolLeft = !useFullWidthLayout && coverPhotoBuffer ? 50 : CANVAS.width - 220;
    composites.push({
      input: platformSymbol,
      top: CANVAS.height - 45 + 1,
      left: platformSymbolLeft,
    });
  }
  
  if (composites.length > 0) {
    // Composite cover first (as background), then other elements
    if (coverPhotoBuffer) {
      // Create base with cover photo
      const coverPos = useFullWidthLayout
        ? { top: 0, left: 0 }
        : { top: 0, left: CANVAS.width - 480 };
      
      composite = sharp(Buffer.from(svgContent))
        .resize(CANVAS.width, CANVAS.height)
        .composite([
          { input: coverPhotoBuffer, ...coverPos },
        ]);
      
      // Now re-apply the SVG overlay and other composites
      const svgOverlayBuffer = await sharp(Buffer.from(svgContent))
        .resize(CANVAS.width, CANVAS.height)
        .png()
        .toBuffer();
      
      const otherComposites = composites.filter(c => c.input !== coverPhotoBuffer);
      
      composite = sharp(coverPhotoBuffer)
        .resize(useFullWidthLayout ? CANVAS.width : 480, useFullWidthLayout ? 300 : CANVAS.height, { fit: 'cover' })
        .extend({
          top: useFullWidthLayout ? 0 : 0,
          bottom: useFullWidthLayout ? CANVAS.height - 300 : 0,
          left: useFullWidthLayout ? 0 : CANVAS.width - 480,
          right: 0,
          background: { r: 10, g: 10, b: 15, alpha: 1 },
        });
      
      // Simpler approach: just composite everything on the SVG base
      composite = sharp(Buffer.from(svgContent))
        .resize(CANVAS.width, CANVAS.height)
