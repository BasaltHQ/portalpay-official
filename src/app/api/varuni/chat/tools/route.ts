...
export function initializeAgent(): VaruniAgent {
  ...
  if (isOn('analytics', true)) register({
    ...
    tools: [
      ...
      // shopAnalytics already returns revenueTrend; separate revenue series tool not needed
      { name: 'back_to_main', description: 'Return to main tools', handler: async () => ({ navigate: 'main' }) },
    ]
  });
  ...
  if (isOn('vendors', true)) register({
    ...
    tools: [
      // Use suppliers to match GraphQL schema; keep tool name for backwards compat
      createGraphQLTool(
        'get_vendors',
