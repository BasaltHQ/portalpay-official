      ...
      const handleOpen = async () => {
        ...
        if (!sessionInitSentRef.current) {
          try {
            ...
            const uiTools: any[] = [
              {
                ...
                function: {
                  name: "addToCart",
                  description: "Add an item to cart by id or sku. Provide either id or sku (id preferred). Qty defaults to 1 if omitted. Use selectedModifiers array to include modifiers.",
                  parameters: {
                    type: "object",
                    properties: {
                      id: { type: "string", description: "Inventory item ID." },
                      sku: { type: "string", description: "Inventory item SKU (used if id not provided)." },
                      qty: { type: "number", minimum: 1, description: "Quantity to add (must be >= 1)." },
                      selectedModifiers: {
                        type: "array",
                        description: "Array of selected modifiers for the item.",
                        items: {
                          type: "object",
                          properties: {
                            ...
                            priceAdjustment: { type: "number", description: "Price adjustment for this modifier." },
                            quantity: { type: "number", description: "Quantity of this modifier (default 1)." },
                          },
                          required: ["groupId", "modifierId", "name"],
                        },
                      },
                      specialInstructions: { type: "string", description: "Optional special instructions for this item." },
                    },
              ...
              },
              {
                ...
                function: {
                  name: "editCartItem",
                  description: "Edit an existing cart item's modifiers by line index. Use this to change modifiers on an item already in the cart without removing and re-adding it.",
                  parameters: {
                    type: "object",
                    properties: {
                      lineIndex: { type: "number", minimum: 0, description: "The 0-based index of the cart line item to edit." },
                      qty: { type: "number", minimum: 1, description: "New quantity (optional, keeps existing if not provided)." },
                      selectedModifiers: {
                        type: "array",
                        description: "New array of selected modifiers to replace existing ones.",
                        items: {
                          type: "object",
                          properties: {
                            groupId: { type: "string", description: "Modifier group ID." },
              ...
              },
              {
                ...
                function: {
                  name: "removeFromCart",
                  description: "Remove a specific item from the cart by its line index. Use getCartSummary first to see cart items and their indices.",
                  parameters: {
                    type: "object",
                    properties: {
                      lineIndex: { type: "number", minimum: 0, description: "The 0-based index of the cart line item to remove." },
                    },
                    required: ["lineIndex"],
                    additionalProperties: false,
                  },
              ...
              },
              {
                ...
                function: {
                  ...
                  description: "Update cart item quantity by id. Set qty to 0 to remove the item from cart.",
                  parameters: {
                    type: "object",
                    properties: {
                      id: { type: "string", description: "Inventory item ID." },
                      qty: { type: "number", minimum: 0, description: "New quantity. Use 0 to remove item." },
                    },
                    additionalProperties: false,
                  },
                },
              ...
              },
              {
                ...
                function: {
                  name: "getCartSummary",
                  description: "Get current cart items with quantities, line totals, and subtotal. Returns items with their line indices for use with editCartItem and removeFromCart.",
                  parameters: { type: "object", properties: {}, additionalProperties: false },
                },
              },
            ];
            ...
            const dynamicInstructions = buildShopConciergePrompt({
              name: shopName,
...
Always prefer tool calls when data is needed; summarize results concisely and accurately.`;
  ...
  const startListening = useCallback(async (opts?: StartOptions) => {
    ...
    try {
      ...
      const preInstructions = buildShopConciergePrompt({
      ...
      if (!hasValidToken) {
        ...
        try {
          const res = await fetch("/api/voice/session", {
            ...
            cache: "no-store",
            body: JSON.stringify({ voice: (opts?.voice || "marin"), wallet: wallet1, instructions: instructionsRef.current || preInstructions }),
          });
